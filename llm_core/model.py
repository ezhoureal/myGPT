...
class Attention(nn.Module):
    def __init__(self, config: Config):
        ...
        self.register_buffer(
            "mask", torch.tril(
                torch.ones(
                    T, T, dtype=torch.float32)).view(
                1, 1, T, T))

    def forward(self, x: torch.Tensor):
        # batch size, time (sequence length), channel (token embedding
        # dimension)
        B, T, C = x.size()
...
class GPT(nn.Module):
    ...
    def init_weight(self, module: nn.Module):
        if module.isinstance(nn.Embedding):
            ...
            if hasattr(module, "RESIDUAL_STD"):
                # times 2 because Attn + MLP counts as 2
    ...
    def forward(self, x, target, startIdx: int = 0):
        ...
        if target is not None:
            loss = nn.functional.cross_entropy(
                next.view(-1, next.shape[-1]), target.view(-1))
        return next, loss
    ...
    def configure_optimizer(
        ...
        fused_available = 'fused' in torch.inspect.signature(
            torch.optim.AdamW).parameters
        fused = fused_available and device == 'cuda'
        extra_args = dict(fused=True) if fused else dict()
        optimizer = torch.optim.AdamW(
            param_groups, lr=learning_rate, betas=(
                0.9, 0.95), eps=1e-8, **extra_args)
    ...
    def from_pretrained(cls, config: Config):
        ...
        for k in sd_keys:
            ...
            if any(k.endswith(postFix) for postFix in transposed):
                ...
                assert sd[k].shape == hf_sd[k].shape, f'key = {k}, sd shape = {
                    sd[k].shape}, hf shape = {
                    hf_sd[k].shape}'
                with torch.no_grad():
                    sd[k].copy_(hf_sd[k])
