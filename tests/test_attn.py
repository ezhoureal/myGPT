from llm_core.model import Config, Attention, AttentionWithKVCache
import torch
import sys
import os
sys.path.insert(
...
def test_attention_output_shape():
    ...
    # Batch size 2, sequence length 16, embedding size 32
    x = torch.randn(2, 16, 32)
    output = attention(x)
    assert output.shape == (
        2, 16, 32), f"Expected shape (2, 16, 32), got {
        output.shape}"
...
def test_kv_cache_matches_attention():
    ...
    assert torch.allclose(output_attention, output_kv_attention,
                          atol=1e-5), "Outputs of Attention and AttentionWithKVCache do not match"
